...
public class Ball : Collidable
{
    ...
    /// <summary>
    /// Helds to track players double tap action
    /// </summary>
    ...
    {
        Rect = new Rect2i(pos.x - (C.BALL_WIDTH / 2), pos.y - (C.BALL_HEIGHT / 2), C.BALL_WIDTH, C.BALL_HEIGHT);
    ...
    public override void Update()
    {
        ...
        {
            if (Time.realtimeSinceStartup - mLastTapTime < 0.5f)
            {
                doubleTap = true;
            }

            mLastTapTime = Time.realtimeSinceStartup;
        }
        ...
        if (StuckToPaddle)
        {
            ...
            Rect.x = paddleRect.center.x - (Rect.width / 2);
            Rect.y = paddleRect.y - (C.BALL_HEIGHT / 2);
            mPos.x = Rect.x;
            mPos.y = Rect.y;
        ...
        else
        {
            ...
            float increment = Math.Min(C.BALL_HEIGHT / 4.0f, C.BALL_WIDTH / 4.0f);
    ...
    public void Render()
    {
        ...
        FES.AlphaSet((byte)((mAlpha / 255.0f) * prevAlpha));

        FES.DrawSprite(FES.SpriteIndex(FlashFrame, 2), new Vector2i(Rect.x, Rect.y));
        FES.AlphaSet(prevAlpha);
    }

    /// <summary>
    /// Bounce the ball off the paddle, this casues a new direction of travel which depends on where the
    ...
    /// </summary>
    /// <returns>Retrun true if ball has hit something</returns>
    private bool DoPhysics()
    {
        ...
        foreach (var brick in game.Level.Bricks)
        {
            CheckCollision(brick, ref bounceX, ref bounceY);
        }

        // Check collisions vs walls
        foreach (var wall in game.Level.Walls)
        {
            CheckCollision(wall, ref bounceX, ref bounceY);
        }

        // Check collision vs paddle
        CheckCollision(game.Level.Paddle, ref bounceX, ref bounceY);

        Vector2i hitPos = Rect.center;
        ...
        if (mHitCollidable.Count == 2)
        {
            ...
            float dist0 = (mHitCollidable[0].Rect.center - Rect.center).ToVector2().sqrMagnitude;
            float dist1 = (mHitCollidable[1].Rect.center - Rect.center).ToVector2().sqrMagnitude;
    ...
    /// <param name="bounceX">Whether we should bounce on X axis as result of the collison</param>
    /// <param name="bounceY">Whether we should bounce on Y axis as result of the collison</param>
    ...
    /// <summary>
    /// Check if ball is "dead", a dead ball is a ball thats out of bounds
    /// </summary>
    private void CheckDeath()
    {
        if (!Dead && mPos.y > FES.DisplaySize.height)
        {
            Dead = true;
            FES.SoundPlay(C.SOUND_DEATH);
        }
    }
}
