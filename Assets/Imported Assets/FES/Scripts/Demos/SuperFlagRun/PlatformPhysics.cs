namespace FESDemo
{
    ...
    public class PlatformPhysics
    {
        ...
        public PlatformPhysics(Entity entity)
        {
            ...
            mPreviousRect = mEntity.ColliderInfo.Rect.Offset(mEntity.Pos);
        }
        ...
        public void Jump()
        {
            ...
            if (mIsOnGround)
            {
                ...
                FES.SoundPlay(SuperFlagRun.SOUND_JUMP, 0.5f, Random.Range(0.9f, 1.1f));
            }
        }
        ...
        public void ApplyVelocity()
        {
            ...
            float incr = FES.SpriteSize().width / 4.0f;
            Vector3 newPos;
        ...
        {
            Rect2i rect = mEntity.ColliderInfo.Rect.Offset(newPos);

            int leftTile = (int)Mathf.Floor((int)rect.min.x / FES.SpriteSize().width) - 1;
            int rightTile = (int)Mathf.Ceil((int)rect.max.x / FES.SpriteSize().width);
            int topTile = (int)Mathf.Ceil((int)rect.min.y / FES.SpriteSize().height) - 1;
            int bottomTile = (int)Mathf.Floor((int)rect.max.y / FES.SpriteSize().height);
            ...
            for (int y = topTile; y <= bottomTile; ++y)
            {
                ...
                {
                    var tile = FES.MapSpriteGet(0, new Vector2i(x, y));
                    if (tile != FES.SPRITE_EMPTY)
                    {
                        var collisionType = FES.MapDataGet<ColliderInfo.ColliderType>(new Vector2i(x, y));
                        ...
                        Rect2i tileRect = new Rect2i(x * FES.SpriteSize().width, y * FES.SpriteSize().height, FES.SpriteSize().width, FES.SpriteSize().height);
                        ...
                        rect = mEntity.ColliderInfo.Rect.Offset(newPos);
            ...
            mPreviousRect = mEntity.ColliderInfo.Rect.Offset(newPos);
